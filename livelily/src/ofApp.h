...
class ofApp : public ofBaseApp
{
	public:
		...
		CmdOutput stripLineFromBar(vector<string> tokens, int lineNum, int numLines);
		void deleteLastBar();
		void deleteLastLoop();
		...
		vector<int> scoreDynamics = {-9, -6, -3, -1, 1, 3, 6, 9};
		// below we also set a default 0 key mapped to 72 MIDI velocity
		map<int, int> mappedScoreDynamics = {{-9, 16}, {-6, 32}, {-3, 48}, {-1, 64}, {0, 72}, {1, 80}, {3, 96}, {6, 112}, {9, 127}};
		...
		// is we start from the first line of a chunk that is not indented
		map<int, vector<string>> noIndentCheck;
