...
vector<int> Editor::getNestDepth()
{
	...
	for (it = allStrings.find(cursorLineIndex); it != allStrings.end(); ++it) {
		...
		if (it->second.substr(0, TABSIZE).compare(tabStr) == 0) {
			...
			if (it->first == cursorLineIndex && ((thisLang == 0 && it->second.find("{") != string::npos) || (thisLang == 1 && it->second.find(":") != string::npos))) continue;
...
void Editor::drawText()
{
	...
	for (int i = lineCountOffset; i < loopIter; i++) {
		...
		if (allStrings[i].size() > 0) {
			...
			vector<string> tokens = tokenizeString(allStrings[i], delimiters[thisLang], true);
		...
		if (executing) {
			...
			switch (thisLang) {
				...
				case 1:
					...
					if (err) {
						setTraceback(3, ((ofApp*)ofGetAppPtr())->sharedData.pyo.getErrorMsg(), executingLineLocal);
					}
					else {
						vector<string> pyStdout = ((ofApp*)ofGetAppPtr())->sharedData.pyo.getStdout();
						if (pyStdout.size() > 1) {
							string s = "";
...
void Editor::upArrow(int lineIndex)
{
	...
	if (setLineIndexesUpward(lineIndex)) {
		...
		if (cursorPos >= ((int)allStrings[cursorLineIndex].size()-allStringStartPos[cursorLineIndex])) {
			...
			if (!inserting && cursorPos == maxCursorPos()) cursorPos--;
		}
...
void Editor::leftArrow()
{
	...
	if (ctrlPressed) {
		...
		while ((int)whiteSpaceIndex >= cursorPos) {
			...
			if (whiteSpaceIndex == string::npos) {
				whiteSpaceIndex = 0;
				break;
			}
		}
...
//--------------------------------------------------------------
void Editor::copyString(int copyTo)
{
	string strToCopy = "";
	if (highlightManyChars) {
		for (int i = topLine; i <= bottomLine; i++) {
			...
			posAndSize[0] += allStringStartPos[i];
			// if we're in normal mode, then we want to include the character under the cursor
			if (!inserting && i == bottomLine) posAndSize[1]++;
...
{
	vector<string> tokens;
	// first check if we do have a string to paste
	switch (pasteFrom) {
		case 0:
			if (ofGetWindowPtr()->getClipboardString().empty()) return;
		case 1:
	...
	switch (pasteFrom) {
		case 0:
			tokens = tokenizeString(ofGetWindowPtr()->getClipboardString(), "\n", false);
			break;
		case 1:
	...
	if (cursorPos < (int)allStrings[cursorLineIndex].size()) {
		remainingStr += allStrings[cursorLineIndex].substr(cursorPos+allStringStartPos[cursorLineIndex]);
	}
	...
	for (string originalToken : tokens) {
		...
		if (i == 0) {
			allStrings[tempCursorLineIndex] = allStrings[tempCursorLineIndex].substr(0, cursorPos+allStringStartPos[tempCursorLineIndex]);
...
void Editor::loadFile(string fileName)
{
	...
	if (!endsWith(fileName, ".lyv") && !endsWith(fileName, ".xml") && !endsWith(fileName, "musicxml") && \
			!endsWith(fileName, ".py") && !endsWith(fileName, ".lua")) {
		fileName += ".lyv";
	}
