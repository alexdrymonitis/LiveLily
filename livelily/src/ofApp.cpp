...
void ofApp::setup(){
	...
	seqVec.tempCurrentScoreLoopIndex = 0;
...
void ofApp::drawScore(){
	if (posCalculator.isThreadRunning()) return;
	ofSetColor(brightness);
...
void ofApp::parseStrings(int index, int numLines){
	...
	if (noErrors) {
		...
		if (storingMultipleBars && numParse == 1) {
			...
			// call parseStrings() to create the loop
	...
	if (commands[0].compare("time") == 0) {
		...
		for (unsigned i = 0; i < patternNames.size(); i++) {
			if (patternName.compare(patternNames[i]) == 0) {
				return "bar/loop/bars already exists";
			}
		}
		...
		string patternName;
		if (commands.size() < 2) {
			return "\\bars command takes a name for a set of bars as an argument";
		}
		patternName = commands[1];
		for (unsigned i = 0; i < patternNames.size(); i++) {
			if (patternName.compare(patternNames[i]) == 0) {
				cout << patternNames[i] << " already exists\n";
				return "bar/loop/bars already exists";
			}
		}
		strayMelodicLine = false;
		if (numParse == 1 && !storePattern) {
			...
			for (unsigned i = 0; i < allBars.size(); i++) {
				if (!i) numBars = allBars[i].size();
					...
					allBars.clear();
					return "all instruments must have the same number of bars";
				}
			}
		...
		string patternName;
		if (commands.size() < 2) {
			return "\\insert command takes a bar name as an argument";
		}
		patternName = commands[1];
		for (unsigned i = 0; i < patternNames.size(); i++) {
			if (patternName.compare(patternNames[i]) == 0) {
		...
		if (commands.size() < 2) {
			return "\\mute command takes at least one argument";
		}
		...
		if (commands.size() < 2) {
			return "\\unmute command takes at least one argument";
		}
		if (commands[1].compare("all") == 0) {
			for (unsigned i = 0; i < seqVec.instruments.size(); i++) {
		...
		if (commands.size() < 2) {
			return "\\mutenow command takes at least one argument";
		}
		if (commands[1].compare("all") == 0) {
			for (unsigned i = 0; i < seqVec.instruments.size(); i++) {
		...
		if (commands.size() < 2) {
			return "\\unmutenow command takes at least one argument";
		}
		if (commands[1].compare("all") == 0) {
			for (unsigned i = 0; i < seqVec.instruments.size(); i++) {
